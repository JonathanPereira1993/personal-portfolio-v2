...
const Header = () => {
  ...
  return (
    <div
      className={`${
        isOnTop ? "top-0 rounded-t-none bg-colorWhite70 dark:shadow-darkMode dark:text-white backdrop-blur-[15px]" : "top-8 backdrop-blur-[2px]"
        ...
        <div className="ml-[40px] flex gap-10">
          {headerLinks.map((link) => (
              <Link
                ...
                className=
                  "dark:text-white group flex items-center gap-1 duration-200 cursor-pointer h-full relative"
                onClick={link.dropdown ? () => setIsSubmenuOpen(!isSubmenuOpen) : null}
              >
                <div className="flex items-center gap-1">
                  <span className="h-full group-hover:opacity-80 dark:hover:opacity-80">{link.name}</span>
                  {link.dropdown && (
                    <FaAngleDown className={`${isSubmenuOpen ? "rotate-180" : ""} ml-1 duration-200 group-hover:opacity-80 dark:hover:opacity-80 text-xs `}/>
                  )}
                </div>
